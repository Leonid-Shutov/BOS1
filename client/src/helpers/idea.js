function generateKeys(key) {
    if (key.length != 16) throw Error('Ключ должен быть 128 бит в длину');

    let keys = new Uint16Array(52);

    let dv = new DataView(key.buffer, key.byteOffset, key.byteLength);
    for (let i = 0; i < 8; i++) keys[i] = dv.getUint16(i);
...
function invertKeys(keys) {
    let invKeys = new Uint16Array(52),
        p = 0;
...
function block(dv, offset, keys) {
    ...
    for (let round = 0; round < 8; round++) {
        let y1 = mul(x1, keys[k++]); 
        let y2 = add(x2, keys[k++]); 
        let y3 = add(x3, keys[k++]); 
        let y4 = mul(x4, keys[k++]); 
...
class CFB {
    crypt(data, keys) {
        ...
        while (offset) block(dv, (offset -= 8), keys);
...
class IDEA {
    ...
    encrypt(data) {
        ...
        return this.mode.encrypt(completeData(data), this.encryptionKeys);
    }

    decrypt(data) {
        let res = this.mode.decrypt(completeData(data), this.decryptionKeys);
